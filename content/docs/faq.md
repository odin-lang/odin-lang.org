...
All source files must have a `package` declaration at the top of the file which share the same name. This `package` declaration at the top of the file exists for consistent ABI, so that link names are deterministic and clear what they are. As Odin is trying to be a form of C alternative, having a deterministic link name is extremely useful for external tooling such as debuggers.
...
Not everything that can be automated ought to be automated. The automation of dependency hell is a case which should not be encouraged. People love to put themselves in hell, dragging others down with them, and a package manager enables that.
