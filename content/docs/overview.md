...
| `ODIN_USE_SEPARATE_MODULES`       | `true` if the `-use-separate-modules` command line switch is passed, which builds each package into its own object file, and then links them together, instead of performing a unity build. |
